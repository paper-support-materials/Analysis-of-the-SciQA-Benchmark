...
def get_similar(element, items=None, embeddings=None, num=None):
    ...
    if num is None or num < 2:
        ...
        scored_texts = []
        for i, score in enumerate(scores[0]):
            scored_texts.append([round(score.item(), 4),
                                 items[i],
                                 embeddings["questions"][i],
                                 embeddings["queries"][i],
                                 embeddings["entities"][i],
                                 embeddings["relations"][i]])
        sorted_scored_texts = sorted(scored_texts, key=lambda x: x[0], reverse=True)
...
def save_embedding():
    ...
    keys = [q["template_id"] for q in train]
    entities = [q["entities"] for q in train]
    relations = [q["relations"] for q in train]
    embeddings = {}
    ...
    embeddings["entities"] = entities
    embeddings["relations"] = relations
...
def runner(filename):
    try:
        ...
        for query in query_list[len(gs):]:
            gq = gpt_query(query, context="", adding="")
            time.sleep(2)
            gq = clean(gq)
            gs.append(gq)
            print(gq)
...
def prepare_rebuttal():
    ...
    b = {}
    data = raw_datasets.get("test")
    b["questions"] = a["queries"]
    b["sparql"] = [x["query"]["sparql"] for x in data]
    b["generated_sparql"] = []
    b["suggestions"] = a["suggestions"]
    save_json("nlp_7_shots.json", b)
